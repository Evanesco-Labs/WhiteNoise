...
func NewCmdHandler(host core.Host, ctx context.Context, actCtx *actor.RootContext) *CmdManager {
	return &CmdManager{
		host:                 host,
		context:              ctx,
		actorCtx:             actCtx,
...
func (manager *CmdManager) ExpendSession(relay core.PeerID, joint core.PeerID, sessionId string) error {
	...
	pl.CommandId = secure.EncodeID(hash[:])
