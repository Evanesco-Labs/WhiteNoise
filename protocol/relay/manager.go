...
type RelayMsgManager struct {
	...
	streamMap         map[string]session.Stream
...
func NewRelayMsgManager(host core.Host, ctx context.Context, actCtx *actor.RootContext, role config.ServiceMode, privateKey crypto.PrivKey) *RelayMsgManager {
	return &RelayMsgManager{
		circuitConnMap:    make(map[string]*CircuitConn),
		secureConnMap:     make(map[string]*secure.SecureSession),
		streamMap:         make(map[string]session.Stream),
...
func (manager *RelayMsgManager) AddCircuitConnCaller(sessionId string, remote core.PeerID) {
	manager.mut.Lock()
	defer manager.mut.Unlock()
...
func (manager *RelayMsgManager) AddCircuitConnAnswer(sessionId string) {
	...
	_, ok := manager.circuitConnMap[sessionId]
	if !ok {
		conn := manager.NewCircuitConn(manager.context, sessionId, "")
		manager.circuitConnMap[sessionId] = conn
	}
}
...
func (manager *RelayMsgManager) RunProbeHandler(ctx context.Context) {
	for {
		...
		if bytes.Equal(p.Rand, probe.Rand) {
			//circuit success
			//data := NewRelayMsg([]byte("build circuit success"), probe.SessionId)
			...
			//circuit build faild, close it
			manager.CloseCircuit(probe.SessionId)
		}
	}
}

//todo: join node time out when building a circuit
//处理超时时注意可能join节点没参与，entry and relay情况:entry会首先断开同join的stream
...
func (manager *RelayMsgManager) AddStream(s session.Stream) {
	manager.mut.Lock()
	defer manager.mut.Unlock()
	manager.streamMap[s.StreamId] = s
}
...
func (manager *RelayMsgManager) SetRole(sessionId string, role common.SessionRole) {
	manager.mut.Lock()
	defer manager.mut.Unlock()
	session, ok := manager.sessionMap[sessionId]
	if !ok {
		return
	}
	session.Role = role
	manager.sessionMap[sessionId] = session
}

func (manager *RelayMsgManager) AddStreamNoSession(id string, s session.Stream) {
	manager.mut.Lock()
	defer manager.mut.Unlock()
	manager.streamMap[id] = s
}

func (manager *RelayMsgManager) GetStream(id string) (session.Stream, bool) {
	manager.mut.Lock()
	defer manager.mut.Unlock()
...
func (manager *RelayMsgManager) RemoveStream(sessionId string, streamId string) {
	manager.mut.Lock()
	defer manager.mut.Unlock()
	...
	for i, stream := range s.Pair {
		if stream.StreamId == streamId {
			s.Pair = append(s.Pair[:i], s.Pair[i+1:]...)
		}
	}
...
func (manager *RelayMsgManager) CloseCircuit(sessionId string) error {
	log.Warnf("Close circuit %v", sessionId)
