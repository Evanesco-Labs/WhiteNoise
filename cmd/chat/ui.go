...
func (ui *ChatUI) refreshPeers() {
	peers := ui.cr.PeerList
	idStrs := make([]string, len(peers))
	for i, p := range peers {
		idStrs[i] = fmt.Sprintf("%s", []byte(p)[:8])
	}
...
func (ui *ChatUI) displayChatMessage(msg *pb.ChatMessage) {
	prompt := withColor("green", fmt.Sprintf("<%s>:", msg.PeerID[:8]))
	...
	for {
		...
		case input := <-ui.inputCh:
			chatMsg, _ := json.Marshal(
				pb.ChatMessage{
					PeerID: ui.service.Host().ID().Pretty(),
					Data:   []byte(input),
				})
			//ui.service.NoiseService.Relay().SendRelay(ui.cr.roomName, relay.NewRelayMsg(chatMsg, ui.cr.roomName))
			conn, ok := ui.service.NoiseService.Relay().GetSecureConn(ui.cr.roomName)
			if !ok {
				continue
			}
			conn.Write(secure.EncodePayload(chatMsg))
		...
		case m := <-ui.cr.Messages:
			...
			if err == nil {
				...
				for _, p := range ui.cr.PeerList {
					if p.Pretty() == peer.ID(chatMsg.PeerID).Pretty() {
						isSave = true
					}
				}

				if isSave == false {
					ui.cr.PeerList = append(ui.cr.PeerList, peer.ID(chatMsg.PeerID))
				}
