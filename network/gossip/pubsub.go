...
import (
	...
	pubsub "github.com/HouMYt/go-libp2p-pubsub"
...
func (service *DHTService) handleNoiseMsg(ctx context.Context) {
	for {
		...
		fut := service.actorCtx.RequestFuture(service.proxyPid, proxy.ReqGetClient{Destination: neg.Destination}, common.RequestFutureDuration)
		res, err := fut.Result()
...
func (service *DHTService) handleGossipMsg(clientInfo proxy.ClientInfo, neg *pb.Negotiate) {
	...
	if joinNode == service.host.ID() {
		...
		//msg := relay.NewRelayMsg([]byte("build circuit success"), neg.SessionId)
	...
	peers := service.host.Peerstore().Peers()

	for i := 0; i < service.RetryTimes; i++ {
		startIndex := rand.New(source).Int()
		for j := 0; j < len(peers); j++ {
			//todo:review this scheme to select relay node
			startIndex++
